...
## Приложение. Эволюция страниц и модулей (дерево)
Принцип: верхний уровень — актуальная страница/модуль. Вложенные уровни — более ранние итерации. Узлы “NGF refactor” выделены отдельно.

### Финансы / сверки
- **Payday2 (актуальная версия)** `/payday2`
  - **NGF refactor Payday2**: разбиение на `index.php` (front controller) + `functions.php` + `ajax.php` + `post.php` + `view.php` + `assets/js` (SoC/MVC)
  - **Payday2beta (промежуточное имя)**: этап перед переименованием в Payday2
  - **Payday (первая версия)** `/payday`
- **Payday3**: в текущем репозитории не обнаружен (если есть отдельная ветка/репо — добавлю в дерево).

### Бронирование
- **TR3 (актуальная публичная бронь)** `/tr3`
  - **NGF refactor TR3**: перенос в изолированную папку `/tr3` + независимый API + MVC‑правила
  - **TR2 (предыдущая версия)** `Tr2.php` + `assets/js/Tr2.js` (страница удалена, артефакты в коде остались)
  - **TableReservation (самая ранняя версия)** `TableReservation.php` + `links/table-reservation.js` (страница удалена, legacy‑код остался)

### Kitchen Analytics (аналитика кухни)
- **Текущий набор экранов**: `kitchen_online` + `dashboard` + `rawdata` + `errors` + `zapara`
  - **NGF refactor Kitchen**: вынос синка/аналитики в `src/classes/KitchenAnalytics.php` + cron‑entrypoints `scripts/kitchen/*`
  - **Ранняя версия**: сбор данных через `cron.php` + базовые страницы `rawdata`/`kitchen_online` (до появления расширенной аналитики)

### Бронирования (админ‑панель)
- **Reservations (актуально)** `/reservations`
  - **NGF refactor Reservations**: вынесение схемы залов в `reservations_hall.js`, разделение view/index, meta‑настройки
  - **Предшественники**: админ‑настройки столов в `admin` + логика схемы из TableReservation/Tr2

### Онлайн‑меню
- **menu‑beta (актуальная публичная витрина)** `/links/menu-beta.php`
  - **NGF refactor меню**: выделение синка в `PosterMenuSync`/`MenuAutoFill` + cron `scripts/menu/cron.php`
  - **Ранняя версия**: базовый вывод категорий/позиций без полноценной i18n‑схемы

### Отчёты
- **Roma / Баня / ЗП**: отдельные модули без самостоятельной эволюционной линейки (точечные итерации внутри своих папок).
...
- Рефакторинг и “переделки” автоматически учитываются, т.к. повторные изменения попадают в те же разделы по изменённым файлам.
...
- Учтённых интервалов: 1186
- Итого эффективного времени: 131 ч 37 мин

### Времязатраты (дерево эволюции страниц)
Правило: время “актуальной страницы” = сумма её времени + всех предшествующих веток эволюции (инклюзивно).

#### Финансы / сверки
- **Payday2 (итого 35 ч 22 мин)**
  - Payday (первая версия): 20 ч 52 мин
  - Payday2 (актуальная версия): 14 ч 30 мин

#### Бронирование
- **TR3 (итого 36 ч 12 мин)**
  - TableReservation (самая ранняя версия): 20 ч 7 мин
  - TR2 (предыдущая версия): 9 ч 54 мин
  - TR3 (актуальная версия): 6 ч 11 мин

#### Reservations
- **Reservations (итого 33 ч 15 мин)**
  - TableReservation (как предшественник схемы/логики): 20 ч 7 мин
  - TR2 (как предшественник схемы/логики): 9 ч 54 мин
  - Reservations (актуальная версия): 3 ч 14 мин

#### Kitchen Analytics
- **Kitchen Analytics suite (итого 14 ч 51 мин)**
  - Telegram alerts/webhook: 5 ч 29 мин
  - kitchen_online: 3 ч 12 мин
  - rawdata: 1 ч 38 мин
  - errors (cooked): 1 ч 30 мин
  - kitchen sync (cron): 1 ч 12 мин
  - dashboard: 1 ч 3 мин
  - zapara: 47 мин

#### Онлайн‑меню
- **menu‑beta / online menu (итого 1 ч 49 мин)**
  - Онлайн-меню: 1 ч 49 мин

#### Отчёты
- **Employees (итого 8 ч 39 мин)**: Отчёты: ЗП сотрудников
- **Баня (итого 5 ч 23 мин)**: Отчёты: Баня
- **Roma (итого 18 мин)**: Отчёты: Roma

### Времязатраты (плоская разбивка по разделам)
Примечание: значения меньше минуты показываем как `<1 мин` (чтобы вкладки/разделы с небольшими правками не выглядели как “0”).

| Раздел | Время |
|---|---:|
| Payday | 20 ч 52 мин |
| TableReservation (legacy) | 20 ч 7 мин |
| Payday2 | 14 ч 30 мин |
| TR2 | 9 ч 54 мин |
| Страницы (прочее) | 9 ч 7 мин |
| Отчёты: ЗП сотрудников | 8 ч 39 мин |
| TR3 (публичная бронь) | 6 ч 11 мин |
| Telegram alerts/webhook | 5 ч 29 мин |
| Отчёты: Баня | 5 ч 23 мин |
| UI/Assets (прочее) | 3 ч 46 мин |
| Reservations | 3 ч 14 мин |
| КухняОнлайн | 3 ч 12 мин |
| Core (DB/Poster/Meta) | 2 ч 49 мин |
| Links (публичные) | 1 ч 50 мин |
| Онлайн-меню | 1 ч 49 мин |
| Rawdata | 1 ч 38 мин |
| Cooked (errors) | 1 ч 30 мин |
| Инфраструктура/Деплой | 1 ч 30 мин |
| Auth/UI shell | 1 ч 24 мин |
| Scripts (прочее) | 1 ч 20 мин |
| Kitchen sync (cron) | 1 ч 12 мин |
| Инфраструктура/Конфиги | 1 ч 8 мин |
| Дашборд | 1 ч 3 мин |
| Docs | 51 мин |
| Zapara | 47 мин |
| Прочее | 18 мин |
...
| Scripts (entrypoints) | 12 мин |
| Neworder | 11 мин |
| Admin/Shell | 11 мин |
| Интеграции: Zalo | 8 мин |
| Admin/Legacy | 7 мин |
| Admin/Access | 6 мин |
| Инфраструктура/Аналитика | 4 мин |
| Admin/Logs | 4 мин |
| Интеграции: WhatsApp | 2 мин |
| Логи/Наблюдаемость | 1 мин |
| Admin/Other | <1 мин |
| Admin/Menu | <1 мин |
| Admin/Sync | <1 мин |
| Admin/Categories | <1 мин |
| Admin/Reservations | <1 мин |
| Admin/Telegram | <1 мин |
| **Итого** | **131 ч 37 мин** |
