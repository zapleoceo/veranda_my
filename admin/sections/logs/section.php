...
function admin_logs_section_state(string &$message): array
{
    ...
    if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST' && (string)($_POST['action'] ?? '') === 'run_sync') {
    ...
    if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST' && (string)($_POST['action'] ?? '') === 'clear_log') {
        if (array_key_exists($view, $logMap)) {
            $path = $logMap[$view];
    ...
    $cronHuman = 'admin_logs_cron_human';
    $syncStatus = 'admin_logs_sync_status';
