...
<div class="card" style="max-width: 1000px; margin: 20px auto 0;">
    <div class="filters" style="display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: 12px;">
    <form method="get" class="filters" style="margin-bottom:0; display: flex; flex-wrap: wrap; gap: 10px; align-items: flex-end; flex: 1;">
        <input type="hidden" name="tab" value="logs">
        <div>
            <label for="view" style="display: block; font-size: 12px; margin-bottom: 4px; color: var(--muted);">Лог</label>
            <select class="in" id="view" name="view" style="padding: 6px 12px; border-radius: 6px; border: 1px solid var(--border);">
        ...
        <div>
            <label for="lines" style="display: block; font-size: 12px; margin-bottom: 4px; color: var(--muted);">Строк</label>
            <input class="in" id="lines" type="number" name="lines" min="50" max="800" value="<?= (int)$lines ?>" style="padding: 6px 12px; border-radius: 6px; border: 1px solid var(--border); width: 80px;">
        </div>
        <button class="btn" type="submit" style="padding: 7px 14px;">Показать</button>
        <div class="muted" style="margin-left:auto; align-self:center; font-size: 12px;">
            <?= is_file($path) ? htmlspecialchars(basename($path)) : 'Файл не найден' ?>
        </div>
    </form>
    <form method="post" style="margin-bottom:0; display: flex; align-items: flex-end;">
        <input type="hidden" name="action" value="clear_log">
        <input type="hidden" name="view" value="<?= htmlspecialchars($view, ENT_QUOTES) ?>">
        <button class="btn-danger" type="submit" style="padding: 7px 14px; background: #fee2e2; color: #991b1b; border: 1px solid #fecaca; border-radius: 6px; cursor: pointer; font-weight: 600;">Очистить</button>
    </form>
    </div>

    <pre style="background: #1e1e1e; color: #d4d4d4; padding: 12px; border-radius: 8px; overflow-x: auto; white-space: pre-wrap; word-wrap: break-word; font-size: 13px; max-height: 70vh; overflow-y: auto;"><?= htmlspecialchars($content !== '' ? $content : '—') ?></pre>
</div>
