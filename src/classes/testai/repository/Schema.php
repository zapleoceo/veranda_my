...
class Schema {
    public function ensure(Database $db, string $tRaw, string $tDaily, string $tSettings, string $tKb): void {
        try {
            ...
            // Migrate: add access column if table existed without it
            try {
                $pdo->exec("ALTER TABLE {$tKb} ADD COLUMN access VARCHAR(20) NOT NULL DEFAULT 'public' AFTER content");
            } catch (\Throwable) {}

            // Migrate: drop old tags column (no longer used, data kept until manual cleanup)
            // intentionally not dropping to preserve existing data
