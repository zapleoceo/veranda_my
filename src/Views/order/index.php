...
<head>
    ...
    <title>Новый заказ — Veranda</title>
    <link rel="stylesheet" href="/assets/css/common.css?v=<?= htmlspecialchars($cssVersion) ?>">
    <link rel="stylesheet" href="/neworder/assets/css/order.css?v=<?= htmlspecialchars($cssVersion) ?>">
</head>
<body class="no-mode">
...
<header class="no-top">
    <div class="no-top__inner">
        <button type="button" class="no-iconbtn" id="noLocationBtn" aria-label="Локация">
            ...
            <span class="no-location-label" id="noLocationLabel">Стол</span>
        </button>
        <div class="no-top__title">Новый заказ</div>
        <button type="button" class="no-iconbtn" id="noMenuRefreshBtn" aria-label="Обновить меню" title="Обновить меню">
    ...
    <div class="no-search-wrap">
        ...
        <input type="search" id="noSearchInput" class="no-search-input" placeholder="Поиск по меню" autocomplete="off">
        <button type="button" class="no-search-clear" id="noSearchClear" hidden aria-label="Очистить">×</button>
    </div>
</header>

<!-- ─── Menu scroll area ─────────────────────────────────────────── -->
<main class="no-menu" id="noMenu">
    <!-- Skeleton loader replaced by JS once menu arrives. -->
...
<button type="button" class="no-cartbar" id="noCartBar" hidden>
    ...
    <span class="no-cartbar__label" id="noCartBarLabel">Корзина</span>
...
<aside class="no-cart" id="noCart" hidden aria-hidden="true">
    ...
    <section class="no-cart__panel" role="dialog" aria-modal="true" aria-labelledby="noCartTitle">
        <header class="no-cart__header">
            <h2 id="noCartTitle">Корзина</h2>
            <button type="button" class="no-iconbtn" data-no-close aria-label="Закрыть">×</button>
        </header>
        ...
        <footer class="no-cart__footer">
            <label class="no-field">
                ...
                <textarea id="noOrderComment" rows="2" placeholder="Например, «без лука»"></textarea>
            </label>
            ...
            <button type="button" class="no-btn no-btn--primary" id="noSubmitBtn" disabled>
                <span class="no-btn__label">Подтвердить заказ</span>
...
<aside class="no-modif" id="noModif" hidden aria-hidden="true">
    ...
    <section class="no-modif__panel" role="dialog" aria-modal="true">
        ...
        <footer class="no-modif__footer">
            ...
            <button type="button" class="no-btn no-btn--primary" id="noModifAdd">Добавить</button>
        </footer>
    </section>
</aside>
...
<aside class="no-success" id="noSuccess" hidden aria-hidden="true">
    <div class="no-success__inner">
        ...
        <h2>Заказ принят</h2>
        <p id="noSuccessSub"></p>
        <button type="button" class="no-btn no-btn--primary" id="noNewOrderBtn">Новый заказ</button>
    </div>
</aside>
...
<script type="module" src="/neworder/assets/js/index.js?v=<?= htmlspecialchars($jsVersion) ?>"></script>
</body>
</html>
