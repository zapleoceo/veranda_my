...
<div class="container sch-wrap">
  ...
  <div class="sch-topbar" data-help-abs="Период просмотра, сводные метрики и действия. Стрелки сдвигают период сохраняя длину; даты задают диапазон руками; пресеты — быстрые шаблоны.">
    ...
    <span class="sch-metric-inline <?= $warningDays > 0 ? 'warn' : '' ?>" data-help-abs="Дней с хотя бы одной проблемой (нет старшего, двойное бронирование, не в графике). Hover на ⚠ в строке дня — конкретная причина."
          title="Дней с предупреждениями"><b><?= $warningDays ?> ⚠</b></span>
...
Hover на ⚠ в конкретном дне покажет причины.">⚠</div>
  ...
  <?php
  // Aggregate hours per employee × their rate. Sorted by ZP DESC.
  $schPayroll = [];   // emp_id => ['name' => ..., 'rate' => ..., 'hours' => ..., 'zp' => ...]
  foreach ($days as $d) {
      foreach ($blocks as $block) {
          foreach ($block['slots'] as $sIdx => $_) {
              $sh = $schGetShift($d['iso'], $block['id'], $sIdx);
              ...
              $eid = (int) ($sh['emp_id'] ?? 0);
              if ($eid <= 0) continue;
              $emp = $empById[$eid] ?? null;
              if (!isset($schPayroll[$eid])) {
                  $schPayroll[$eid] = [
                      'id'    => $eid,
                      'name'  => $emp['name'] ?? ($sh['emp_name'] ?? "uid:$eid"),
                      'tag'   => $emp['tag'] ?? '',
                      'rate'  => (int) ($emp['rate_per_hour'] ?? 0),
                      'hours' => 0.0,
                  ];
              }
              $schPayroll[$eid]['hours'] += $hrs;
          }
      }
  }
  foreach ($schPayroll as &$row) $row['zp'] = $row['hours'] * $row['rate'];
  unset($row);
  usort($schPayroll, static fn ($a, $b) => $b['zp'] <=> $a['zp']);
  ?>
  <section class="sch-payroll-section"
    ...
    <table class="sch-payroll-table" id="schPayrollTable">
      ...
      <tbody>
        <?php foreach ($schPayroll as $row): ?>
          <tr data-emp-id="<?= (int) $row['id'] ?>">
            <td class="who"><?= htmlspecialchars($row['name']) ?></td>
            <td class="tag"><?= htmlspecialchars($row['tag']) ?></td>
            <td class="num"><span class="hours"><?= number_format($row['hours'], 1, '.', '') ?></span></td>
            <td class="num"><span class="rate"><?= $row['rate'] > 0 ? number_format($row['rate'], 0, '.', ' ') : '—' ?></span></td>
            <td class="num zp"><span class="zp-val"><?= $row['zp'] > 0 ? number_format($row['zp'] / 1_000_000, 2, '.', '') . 'M' : '—' ?></span></td>
          </tr>
        <?php endforeach; ?>
        <?php if (empty($schPayroll)): ?>
          <tr><td colspan="5" class="empty">Нет смен за выбранный период.</td></tr>
        <?php endif; ?>
      </tbody>
      <tfoot>
        <tr>
          <td colspan="2">Итого</td>
          <td class="num"><span data-total="hours"><?= number_format(array_sum(array_column($schPayroll, 'hours')), 1, '.', '') ?></span></td>
          <td class="num">—</td>
          <td class="num zp"><span data-total="zp"><?= ($t = array_sum(array_column($schPayroll, 'zp'))) > 0 ? number_format($t / 1_000_000, 2, '.', '') . 'M' : '—' ?></span></td>
        </tr>
      </tfoot>
    </table>
  </section>
  ...
  <section class="sch-coverage-section"
    ...
    foreach ($schColorsPresent as $c) {
        ...
        foreach ($blocks as $b) {
            ...
            // Same overlay logic as the header row above: prefer Poster
            // hall name for hall-bound blocks.
  ...
  foreach ($days as $d) {
      $schStatsPayload['days'][] = [
          // iso required by JS recomputeHeatmapFromState() — it walks
          // App.state.shifts[iso] to rebuild the per-day hour buckets
...
<script src="/schedule/assets/js/schedule.js?v=20260520_btnstate" defer></script>
