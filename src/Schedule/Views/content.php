...
Hover на ⚠ в конкретном дне покажет причины.">⚠</div>
  ...
  <div class="sch-snapshots" data-help-abs="Именованные версии графика. Кнопкой «Сохранить версию» создаётся именованная точка восстановления. Клик по версии — загрузить. Hover на pill: ✏ — переименовать, × — удалить. Автосохранения сюда не попадают — они просто обновляют текущий черновик.">
    <span class="sch-snap-label">Версии:</span>
    <?php foreach ($snapshots as $snap):
        $when = preg_replace('/^(\d{4})-(\d{2})-(\d{2}) (\d{2}:\d{2}).*$/', '$3.$2 $4', (string)$snap['created_at']);
    ?>
      <span class="sch-snap-pill" data-snap-id="<?= (int)$snap['id'] ?>" data-snap-label="<?= htmlspecialchars($snap['label']) ?>">
...
<script src="/schedule/assets/js/schedule.js?v=20260520_blocknames" defer></script>
