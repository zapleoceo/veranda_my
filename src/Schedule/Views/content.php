...
<div class="container sch-wrap">
  ...
  // Identify the editor by login e-mail — unambiguous, no name lookup.
  $lockHolderNm  = (string) ($lockHolder['email'] ?? $lockHolder['name'] ?? 'другой пользователь');
  // "с HH:MM" (or "с DD.MM HH:MM" if not today) from acquired_at — lets the
  // viewer judge whether it's an active session or a forgotten-open tab.
  $lockSince = '';
  if (preg_match('/^(\d{4})-(\d{2})-(\d{2})\s+(\d{2}):(\d{2})/', (string) ($lockHolder['acquired_at'] ?? ''), $mLk)) {
      $lockSince = ($mLk[1] . '-' . $mLk[2] . '-' . $mLk[3] === date('Y-m-d'))
          ? "с {$mLk[4]}:{$mLk[5]}"
          : "с {$mLk[3]}.{$mLk[2]} {$mLk[4]}:{$mLk[5]}";
  }
  ?>
  <div class="sch-lock-banner" id="schLockBanner"<?= $lockOwnedBoot ? ' hidden' : '' ?>>
    <span class="sch-lock-icon">🔒</span>
    <span class="sch-lock-text">
      График сейчас редактирует <b id="schLockHolderName"><?= htmlspecialchars($lockHolderNm) ?></b><span id="schLockHolderMeta" class="sch-lock-meta"><?= $lockSince !== '' ? ' (' . htmlspecialchars($lockSince) . ')' : '' ?></span>.
      Чтобы редактировать самому — попроси его выйти со страницы или нажми «Перехватить».
    ...
    <button type="button" class="sch-btn" id="schLockSteal" title="Перехватить редактирование — если кто-то забыл закрыть вкладку">Перехватить</button>
  </div>
...
Hover на ⚠ в конкретном дне покажет причины.">⚠</div>
  ...
  <section class="sch-coverage-section"
    ...
    // we squeeze into each cell. Order = first-appearance in $blocks so
    // the heatmap legend matches the order users see in the main grid
    // (avoids the "Старшие смены | Banya | Veranda" vs "Старшие смены |
    // Veranda | Banya" mismatch).
    $schColorsPresent = [];
    foreach ($blocks as $b) {
        $c = BlockColor::of($b);
  ...
  <div class="sch-snapshots" data-help-abs="Именованные версии графика. Кнопкой «Сохранить версию» создаётся именованная точка восстановления. Клик по версии — загрузить. Hover на pill: 🔗 — публичная ссылка (read-only, без ЗП), ✏ — переименовать, × — удалить. Автосохранения сюда не попадают — они просто обновляют текущий черновик.">
    ...
    ?>
      <span class="sch-snap-pill" data-snap-id="<?= (int)$snap['id'] ?>" data-snap-label="<?= htmlspecialchars($snap['label']) ?>" data-share-url="<?= htmlspecialchars($shareUrl) ?>">
        ...
        <?php if ($author !== ''): ?><span class="sch-snap-by" title="Автор версии: <?= htmlspecialchars($author) ?>">👤 <?= htmlspecialchars($author) ?></span><?php endif; ?>
        <?php if ($shareUrl !== ''): ?>
...
<!-- ════════ Modal: save conflict (draft version moved under us) ════════ -->
<div id="schModalConflict" class="sch-modal-backdrop" role="dialog" aria-modal="true">
  <div class="sch-modal">
    <h3>⚠ График изменили, пока ты редактировал</h3>
    <p class="sch-modal-hint" id="schConflictWho" style="margin-bottom: 12px; font-size: 12.5px;">—</p>
    <p class="sch-modal-hint" style="margin-bottom: 4px;">Выбери, что сделать с правками в этой вкладке — <b>ничего не пропадёт без твоего решения</b>:</p>
    <div class="sch-modal-actions" style="flex-direction: column; align-items: stretch; gap: 8px;">
      <button class="sch-btn primary" id="schConflictOverwrite">💾 Сохранить мою версию (перезаписать чужую)</button>
      <button class="sch-btn" id="schConflictReload">↻ Взять актуальную (мои правки в этой вкладке пропадут)</button>
      <button class="sch-btn ghost" id="schConflictCancel">Пока ничего не делать (посмотреть/скопировать)</button>
    </div>
  </div>
</div>

<script src="/schedule/assets/js/schedule.js?v=20260628_conflictui" defer></script>
