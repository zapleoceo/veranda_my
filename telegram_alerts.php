...
try {
    ...
    foreach ($dueWa as $row) {
        ...
        $waBridgeSend($phone, implode("\n", $lines));
        try {
            $db->query("UPDATE {$waStates} SET reminder_sent_at = ? WHERE code = ?", [date('Y-m-d H:i:s'), $code]);
        } catch (\Throwable $e) {}
        $remindedWa++;
    }

    if ($remindedTg > 0 || $remindedWa > 0) {
        $logLine('LinkReminders tg=' . (int)$remindedTg . ' wa=' . (int)$remindedWa);
